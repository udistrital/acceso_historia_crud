...
type AccesoHistoriaClinicaController struct {
	beego.Controller
}

// URLMapping ...
func (c *AccesoHistoriaClinicaController) URLMapping() {
...
func (c *AccesoHistoriaClinicaController) Post() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddAccesoHistoriaClinica(&v); err == nil {
...
func (c *AccesoHistoriaClinicaController) GetOne() {
	...
	v, err := models.GetAccesoHistoriaClinicaById(id)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		c.Data["json"] = v
...
// @Failure 403
// @router / [get]
func (c *AccesoHistoriaClinicaController) GetAll() {
	var fields []string
	var sortby []string
	var order []string
...
func (c *AccesoHistoriaClinicaController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.AccesoHistoriaClinica{IdAccesoHistoria: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateAccesoHistoriaClinica(&v); err == nil {
			c.Data["json"] = "OK"
		} else {
			c.Data["json"] = err.Error()
		}
...
func (c *AccesoHistoriaClinicaController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteAccesoHistoriaClinica(id); err == nil {
		c.Data["json"] = "OK"
	} else {
		c.Data["json"] = err.Error()
	}
